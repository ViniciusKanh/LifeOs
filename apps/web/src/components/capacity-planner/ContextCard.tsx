...
/** Emoji de condição — leitura rápida do clima, sem inventar dado além do que o Contexto do Dia já calcula. */
function conditionEmoji(favorable: boolean | null): string {
  if (favorable == null) return "🌤️";
  return favorable ? "☀️" : "🌧️";
}
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <p className="text-sm font-semibold">{conditionEmoji(context.favorable)} Contexto do dia</p>
      </div>
      {context.temperature != null ? (
        <>
          ...
          <p className="text-xs mt-1 font-medium" style={{ color: context.favorable ? "#2E7D6B" : "#C9821E" }}>
            {context.favorable ? "✅ Condições favoráveis" : "⚠️ Condições exigem atenção"}
          </p>
        </>
      ) : (
