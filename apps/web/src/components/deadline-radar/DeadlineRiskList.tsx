...
import { ChevronRight, TrendingUp, GraduationCap } from "lucide-react";
...
export function DeadlineRiskList({ risks }: { risks: ProjectRiskItem[] }) {
  return (
    <Card className="p-5">
      ...
      <p className="text-xs text-slate mb-4">Projetos e trabalhos acadêmicos, pelo seu ritmo atual de progresso.</p>
      {risks.length === 0 ? (
        <p className="text-sm text-slate py-6 text-center">Nenhum projeto ou trabalho acadêmico com prazo para avaliar risco ainda.</p>
      ) : (
        <ul className="space-y-3">
          {risks.map((r) => (
            <li key={`${r.kind}:${r.projectId}`} className="flex items-center gap-3">
              {r.kind === "academic" ? (
              ...
              <Link to={r.sourceModule} className="p-1 shrink-0" aria-label={`Ver ${r.projectName}`}>
