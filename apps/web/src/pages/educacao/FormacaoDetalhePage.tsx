...
export function FormacaoDetalhePage() {
  ...
  const dashboard = useEducationDashboard(id);
  ...
  const handleAddChecklistItem = async () => {
    ...
    await dashboard.createChecklistItem({ title: checklistTitle.trim() });
    setChecklistTitle("");
  };
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8">
      ...
      <Card className="p-5 md:p-6 mb-4">
        <div className="flex flex-col lg:flex-row lg:items-center gap-5">
          ...
          <div className="flex items-center gap-5 text-xs shrink-0">
            ...
            <div>
              <p className="text-slate mb-1">Status</p>
      ...
      {/* ==================== Projetos acadêmicos / Semestre / Motivação ==================== */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
          ...
          {education.academicProjects.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {education.academicProjects.map((p) => (
                <a key={p.id} href="#projetos-academicos" className="flex items-center justify-between gap-2 rounded-lg p-2 hover:bg-paper dark:hover:bg-ink transition-colors">
                  <div className="min-w-0">
        ...
        <Card className="p-4">
          ...
          {dashboard.checklist.length === 0 ? (
            <p className="text-xs text-slate mb-3">Nenhum item cadastrado ainda.</p>
          ) : (
            <div className="space-y-2 mb-3">
              {visibleChecklist.map((item) => (
                <button
                  key={item.id}
                  onClick={() => dashboard.updateChecklistItem({ itemId: item.id, patch: { done: !item.done } })}
                  className="w-full flex items-center gap-2.5 text-left group"
                >
                  <span
                    className={`w-4 h-4 rounded shrink-0 border flex items-center justify-center ${
                      item.done ? "bg-growth border-growth text-white" : "border-paper-border dark:border-ink-border"
                    }`}
                  >
                    {!!item.done && <CheckSquare size={11} />}
                  </span>
                  <span className={`text-xs flex-1 truncate ${item.done ? "line-through text-slate" : ""}`}>{item.title}</span>
                  {item.due_date && <span className="text-[10px] text-slate shrink-0">{formatDayMonth(item.due_date).day}/{formatDayMonth(item.due_date).month}</span>}
                </button>
              ))}
              {dashboard.checklist.length > CHECKLIST_LIMIT && (
                <button onClick={() => setShowAllChecklist((v) => !v)} className="text-[11px] text-brand-600 dark:text-brand-500 font-medium">
                  {showAllChecklist ? "Ver menos" : "Ver todos →"}
                </button>
              )}
            </div>
          )}
          <div className="flex gap-1.5">
            <input
              ...
              placeholder="Novo item do semestre..."
              className="flex-1 rounded-lg px-2.5 py-1.5 text-xs bg-paper dark:bg-ink outline-none border border-paper-border dark:border-ink-border focus:border-brand-500"
            />
            <button onClick={handleAddChecklistItem} className="rounded-lg px-2.5 border border-paper-border dark:border-ink-border text-slate">
              <Plus size={13} />
            </button>
          </div>
        </Card>

        <div className="rounded-2xl p-5 bg-gradient-to-br from-cat-green/90 to-brand-500 text-white flex flex-col justify-between">
          <div className="flex items-center gap-2 mb-2">
            <Sprout size={18} />
            <ListChecks size={16} className="opacity-70" />
          </div>
          <p className="font-display font-semibold text-lg leading-snug">"Aprendizado é um investimento que sempre rende."</p>
        </div>
      </div>
      ...
      {subjectModalOpen && (
        <QuickSubjectModal
          ...
          onCreate={dashboard.quickCreateSubject}
        />
      )}
      ...
      {deadlineModalOpen && (
        <DeadlineModal
          ...
          onCreate={dashboard.createDeadline}
        />
      )}
      ...
      {studyModalOpen && (
        <StudySessionModal
          ...
          onCreate={dashboard.logStudySession}
        />
      )}
    </div>
  );
    ...
    <div>
      ...
      {modalState.open && (
        <TaskModal
          ...
          statusOptions={ACADEMIC_COLUMNS}
          onClose={close}
          onSave={handleSave}
          onDelete={removeTask}
        />
